...
def calculate_molecular_descriptors(mol):
    d = {
        ...
        "NumAtoms": mol.GetNumAtoms(), "NumHeavyAtoms": Lipinski.HeavyAtomCount(mol),
        "NumHeteroatoms": Lipinski.NumHeteroatoms(mol), "NumBonds": mol.GetNumBonds(),
        ...
        "BertzCT": Descriptors.BertzCT(mol), "Chi0v": Descriptors.Chi0v(mol),
        "Chi1v": Descriptors.Chi1v(mol), "Kappa1": Descriptors.Kappa1(mol),
        "Kappa2": Descriptors.Kappa2(mol), "Kappa3": Descriptors.Kappa3(mol),
        "MolMR": Crippen.MolMR(mol), "PEOE_VSA1": Descriptors.PEOE_VSA1(mol),
        "PEOE_VSA2": Descriptors.PEOE_VSA2(mol), "SMR_VSA1": Descriptors.SMR_VSA1(mol),
        "SMR_VSA5": Descriptors.SMR_VSA5(mol), "SlogP_VSA1": Descriptors.SlogP_VSA1(mol),
        ...
        "LabuteASA": Descriptors.LabuteASA(mol), "BalabanJ": Descriptors.BalabanJ(mol),
        "HallKierAlpha": Descriptors.HallKierAlpha(mol),
        "MolLogP_Crippen": Crippen.MolLogP(mol),
...
def build_feature_vector(smiles, columns, n_features):
...
def compute_ad(query_fingerprint, training_matrix):
    ...
    closest_similarity = float(max(similarities))
...
def main():
    ...
    columns, n_features = load_feature_info(args.feature_info)
    model = joblib.load(args.model)
    training_matrix = np.load(DEFAULT_AD_MATRIX, allow_pickle=False)
    if training_matrix.ndim != 2 or training_matrix.shape[1] != FP_NBITS:
        raise ValueError(f"{DEFAULT_AD_MATRIX} must have shape (n, {FP_NBITS}); got {training_matrix.shape}")
    training_matrix = training_matrix.astype(np.uint8, copy=False)

    if args.smiles is not None:
        smiles_list = [args.smiles.strip()]
    else:
        with open(args.smiles_file, encoding="utf-8") as f:
            smiles_list = [line.strip() for line in f if line.strip() and not line.lstrip().startswith("#")]

    rows = []
    for smiles in smiles_list:
        vector, fingerprint = build_feature_vector(smiles, columns, n_features)
        if vector is None:
        ...
        distance, within_domain = compute_ad(fingerprint, training_matrix)
    ...
    pd.DataFrame(rows, columns=["SMILES", "Prediction", "Probability", "AD_ClosestDistance", "AD_WithinDomain"]).to_csv(args.out, index=False)
